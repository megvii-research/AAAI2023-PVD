...
class _march_rays_train(Function):
    ...
    def forward(ctx, rays_o, rays_d, bound, density_bitfield, C, H, nears, fars, step_counter=None, mean_count=-1, perturb=False, align=-1, force_all_rays=False, dt_gamma=0, max_steps=1024):
        ...
        # The third curriculum according to the weight of point. Therefore the return may contain the weights and indexs for fast seclecting hard points.
        # 指定没条ray上最多记录的点数，比如64.在ray的rendering过程,使用插入排序选择最64个值最大的weight，记录下其值和在4096条ray上所有点上的index。
...
class _composite_rays_train(Function):
    ...
    def forward(ctx, sigmas, rgbs, deltas, rays):
        ...
        weighty_weights = (torch.zeros(M, dtype=sigmas.dtype, device=sigmas.device) - 1).contiguous()
        weighty_weights_index = (torch.zeros(M, dtype=torch.int32, device=sigmas.device) - 1).contiguous()
        third_curri_Ratio = 0.3  # 0.3

        _backend.composite_rays_train_forward(sigmas, rgbs, deltas, rays, M, N, weights_sum, depth, image, weighty_weights, weighty_weights_index, third_curri_Ratio)

        ctx.save_for_backward(sigmas, rgbs, deltas, rays, weights_sum, depth, image)
        ctx.dims = [M, N]
        return weights_sum, depth, image, weighty_weights, weighty_weights_index

    @staticmethod
    @custom_bwd
    def backward(ctx, grad_weights_sum, grad_depth, grad_image, grad_weighty_weights, grad_weighty_weights_index):
        grad_weights_sum = grad_weights_sum.contiguous()
        grad_depth = grad_depth.contiguous()
        grad_image = grad_image.contiguous()
        # print(grad_depth.abs().max().item(), grad_image.abs().max().item())
        ...
        _backend.composite_rays_train_backward(grad_weights_sum, grad_depth, grad_image, sigmas, rgbs, deltas, rays, weights_sum, depth, image, M, N, grad_sigmas, grad_rgbs)
