...
def rand_poses(size, device, radius=1, theta_range=[np.pi/3, 2*np.pi/3], phi_range=[0, 2*np.pi]):
    ...
    def normalize(vectors):
        return vectors / (torch.norm(vectors, dim=-1, keepdim=True) + 1e-10)
    interval_nums = torch.tensor([i * 1 / (size-1) for i in range(size)], dtype=torch.float32, device=device)
    ...
    forward_vector = - normalize(centers)
    up_vector = torch.FloatTensor([0, -1, 0]).to(device).unsqueeze(0).repeat(size, 1) # confused at the coordinate system...
    right_vector = normalize(torch.cross(forward_vector, up_vector, dim=-1))  # cross product
    up_vector = normalize(torch.cross(right_vector, forward_vector, dim=-1))
    ...
    def __init__(self, opt, device, type='train', downscale=1, n_test=10):
        ...
        if True:
            ...
            for f in tqdm.tqdm(frames, desc=f'Loading {type} data:'):
                ...
                if self.mode == 'blender' and f_path[-4:].lower() != '.png' and f_path[-4:].lower() != '.jpg' :
                    f_path += '.png' # so silly...
