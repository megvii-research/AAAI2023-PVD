...
class NeRFRenderer(nn.Module):
    ...
    def run_cuda(self, rays_o, rays_d, dt_gamma=0, bg_color=None, perturb=False, force_all_rays=False, max_steps=1024, inherited_params=[], **kwargs):
        # rays_o, rays_d: [B, N, 3], assumes B == 1
        # return: image: [B, N, 3], depth: [B, N]
        ...
        if self.training:  # different with testing
            ...
            self.local_step += 1
            if self.args.render_stu_first:  # if stu first, then using stu to calculate xyzs, and tea will inherite the xyzs
                '''
                关于xyzs, dirs, deltas, rays的说明：
                    xyzs, dirs是根据rays_o和rays_d在ray进行采样得到的所有空间点
                    rays: 标记了点属于哪一条ray，shape为(总点数，3)，其中3元组数的含义是:[ray的标号idx, 点的起始index, 该ray上总共采点数量]
                          即xyzs[rays[i, 1]:rays[i,1]+rays[i, 2]] --> points belonging to rays[i, 0]
                    deltas: [总点数, 2], all generated points' deltas. (first for RGB, second for Depth)
                '''
                if not self.is_teacher:
                    xyzs, dirs, deltas, rays = raymarching.march_rays_train(rays_o, rays_d, self.bound, self.density_bitfield, self.cascade, self.grid_size, nears, fars, counter, self.mean_count, perturb, 128, force_all_rays, dt_gamma, max_steps)
                    inherited_params = [xyzs, dirs, deltas, rays]
                else: xyzs, dirs, deltas, rays = inherited_params
            else:
                if self.is_teacher:
                    xyzs, dirs, deltas, rays = raymarching.march_rays_train(rays_o, rays_d, self.bound, self.density_bitfield, self.cascade, self.grid_size, nears, fars, counter, self.mean_count, perturb, 128, force_all_rays, dt_gamma, max_steps)
                    inherited_params = [xyzs, dirs, deltas, rays]
            ...
            if len(sigmas.shape) == 2:
                ...
                images = []
                for k in range(K):
                    weights_sum, depth, image, weighty_weights, weighty_weights_index = raymarching.composite_rays_train(sigmas[k], rgbs[k], deltas, rays)
                    image = image + (1 - weights_sum).unsqueeze(-1) * bg_color
